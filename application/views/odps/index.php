          ...
          <table class="table table-hover" id="dtHorizontalExample" cellspacing="0">
            ...
            <tbody style="cursor:pointer;" id="tbody">
              ...
              foreach ($odps as $data) : ?>
                <tr>
                  <td onclick="detail_odps('<?= $data->id_odp ?>')"><?= $no++ ?>.</td>
                  <td onclick="detail_odps('<?= $data->id_odp ?>')"><img src="<?= base_url() ?>assets/upload/odp/<?= $data->foto_odp ?>" width="100px" alt=""></td>
                  <td onclick="detail_odps('<?= $data->id_odp ?>')"><?= $data->kode_odp ?></td>
                  <?php if ($this->session->userdata('login_session')['level'] == 'admin') : ?>
                    <td onclick="detail_odps('<?= $data->id_odp ?>')"><?= $data->nama_teknisi ?></td>
                  <?php endif; ?>
                  <td onclick="detail_odps('<?= $data->id_odp ?>')"><?= $data->lokasi ?></td>
                  <td onclick="detail_odps('<?= $data->id_odp ?>')"><?= date('d F Y', strtotime($data->tgl_pendataan)) ?></td>
                  <td onclick="detail_odps('<?= $data->id_odp ?>')"><?= $data->status ?></td>
