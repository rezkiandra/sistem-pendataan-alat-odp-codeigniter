...
<div class="container-fluid">
  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div>
      <?php if ($this->session->userdata('login_session')['level'] == 'admin' && $odp != null) : ?>
              ...
              foreach ($odp as $data) : ?>
                <tr>
                  ...
                  <?php if ($this->session->userdata('login_session')['username'] == $data->nama_teknisi || $this->session->userdata('login_session')['level'] == 'admin' || $this->session->userdata('login_session')['level'] == 'pegawai') : ?>
                    <td>
                      <center>
                        <a href="<?= base_url() ?>odp/pelihara/<?= $data->id_odp ?>" class="btn btn-info btn-sm">
                          <i class="fas fa-heart"></i>
                        </a>
                        <a href="<?= base_url() ?>odp/ubah/<?= $data->id_odp ?>" class="btn btn-warning btn-sm">
                          <i class="fas fa-pen"></i>
                        </a>
                        <a href="javascript:void(0)" onclick="konfirmasi('<?= $data->id_odp ?>')" class="btn btn-danger btn-sm">
